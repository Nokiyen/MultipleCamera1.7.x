package noki.multiplecamera.tile;
...
/**********
 * @class TileMultipleCamera
 *
 * @description
 * @description_en
 * 
 * @see BlockMultipleCamera.
 */
...
/*	public TileMultipleCamera(World world, int metadata) {
	...
	public void createCamera() {
		...
		this.cameraId = CameraManagerServer.addCamera(this.worldObj,
				(double)this.xCoord+0.5D, (double)this.yCoord-1.0D, (double)this.zCoord+0.5D, this.getBlockMetadata()*90, 0,
				this.uuid, false);
		this.activated = true;
	...
	public void resetCameraInfo() {
		...
		camera.setCameraInfo(this.worldObj,
				(double)this.xCoord+0.5D, (double)this.yCoord-1.0D, (double)this.zCoord+0.5D, this.getBlockMetadata()*90, 0F);
		...
		if(this.cameraPlayer == null) {
			...
			this.cameraPlayer.setLocationAndAngles((double)this.xCoord+0.5D, (double)this.yCoord-1.0D, (double)this.zCoord+0.5D,
					this.yaw, this.pitch);
			this.cameraPlayer.setPositionAndRotation((double)this.xCoord+0.5D, (double)this.yCoord-1.0D, (double)this.zCoord+0.5D,
					this.yaw, this.pitch);
			if(this.cameraPlayer == null) {
	...
	// 毎tickの判定で、ワールドイン(ゲーム開始)時、あるいはチャンクロード時にカメラをセット。
	// 本来的には無駄な処理だが他に上手い方法が思いつかない。
	@Override
